# Import flask and flask-extensions
from flask import Flask, render_template, flash, abort
from flask_mail import Mail, Message
from werkzeug.security import generate_password_hash, check_password_hash
from flask_login import login_required, login_user, logout_user, current_user
# Import other libraries
from base64 import b64encode
import os
from threading import Thread
# Import my files
from db import db, migrate, Users, Books, Reservations
from forms import RegistrationForm, LoginForm
from login import manager, load_user
from user import User
# Create app and import config
app = Flask(__name__)
app.config.from_pyfile('config.py')
# Create mail instance
mail = Mail(app)
# Init extensions with app
manager.init_app(app)
db.init_app(app)
migrate.init_app(app, db)

# The function of sending a letter through a thread


def async_send_mail(app, msg):
    with app.app_context():
        mail.send(msg)

# The function of creating a letter and a thread for faster functioning


def send_mail(subject, recipient, template, **kwargs):
    msg = Message(
        subject,      sender=app.config['MAIL_DEFAULT_SENDER'],  recipients=[recipient])
    msg.html = render_template(template,  **kwargs)
    thr = Thread(target=async_send_mail,  args=[app,  msg])
    thr.start()
    return thr

# Index page


@app.route('/', methods=['GET'])
def index():
    # Return index page
    return render_template('index.html')

# Registration page


@app.route('/registration', methods=['GET', 'POST'])
def registration():
    # Create form
    form = RegistrationForm()
    # POST request
    if form.validate_on_submit():
        # Send error message if email in users' emails
        if form.email.data in [u.email for u in Users.query.all()]:
            flash('Такой аккаунт уже существует', category='error')
        else:
            try:
                # Create user and send to the DB
                user = Users(name=form.name.data, email=form.email.data, password=generate_password_hash(form.password.data),
                             age=form.age.data, verified=False, access_key=b64encode(os.urandom(50)).decode('utf-8').replace('/', ''))

                db.session.add(user)
                db.session.commit()
                # Send email to the user's email
                send_mail(subject="Регистрация Penza Lib", recipient=form.email.data, template='mail.html',
                          name=form.name.data, key=Users.query.filter_by(email=form.email.data).first().access_key)
                # Send success message
                flash(
                    'На вашу почту было отправлено письмо с подтверждением', category='success')
            # Rollback if any occurs
            except Exception as e:
                print(e)
                db.session.rollback()
    # Return registration page
    return render_template('registration.html', form=form)

# The function of completing registration by the link from the letter


@app.route('/accept/<key>', methods=['GET', 'POST'])
def accept(key):
    try:
        # Change user.verified to True and login user
        user = Users.query.filter_by(access_key=key).first()
        user.verified = True
        db.session.commit()
        userlogin = User().create(user)
        login_user(userlogin, remember=True)
        return f'Успешная регистрация!'
    # Rollback and abort 404 if any occurs
    except Exception as e:
        print(e)
        db.session.rollback()
        abort(404)

# Login page


@app.route('/login', methods=['GET', 'POST'])
def login():
    # Create form
    form = LoginForm()
    # Get user data by email
    user = Users.query.filter_by(email=form.email.data).first() if form.email.data in [
        u.email for u in Users.query.all()] else None
    # POST request
    if form.validate_on_submit():
        # Login user if data is true
        if user and check_password_hash(form.password.data, user.password):
            userlogin = User().create(user)
            login_user(userlogin, remember=form.remember.data)
        # Send error message if data is wrong
        else:
            flash('Неправильные данные', category=True)
    # Return login page
    return render_template('login.html', form=form)


# Run app
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True)
